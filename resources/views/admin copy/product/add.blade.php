        ...
        <div class="row">
          ...
          <div class="col-md-4 plusBoxWrap relative">
            <div class="form-group">
              ...
              <select name="drugstore" id="drugstore" class="form-control form-inputtext" required>
                <option value="">Select Drugstore</option>
                @if(count($data['drugstore'])>0)
                @foreach($data['drugstore'] as $row)
                <option value="{{$row->id}}" {{ ($data['drugstore_id'] == $row->id ? "selected":"") }} >{{$row->name}} / {{$row->email}}</option>
                @endforeach
                @endif
              </select>
              @error('drugstore')
