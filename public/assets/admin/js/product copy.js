...
$(document).on("keyup", ".cost_rate_old", function () {
    var row_id = $(this).attr("id").split("cost_rate_")[1];
    ...
    var costgstamt = (
        (Number(costrate) * Number(costgstper)) /
        Number(100)
    ).toFixed(4);
    ...
    var cost_rate_with_extracha = (
        Number(costrate) + Number(extra_charge)
    ).toFixed(4);
    var productrs = (
        (Number(cost_rate_with_extracha) * Number(productper)) /
        Number(100)
    ).toFixed(4);
    if (productrs <= 0) {
    ...
    var sellingprice = (
        Number(cost_rate_with_extracha) + Number(productrs)
    ).toFixed(4);

    $("#selling_price_" + row_id).val(sellingprice);
    var sellingper = $("#sell_gst_percent_" + row_id).val();
    var sellingstrs = (
        (Number(sellingprice) * Number(sellingper)) /
        Number(100)
    ).toFixed(4);
...
$(document).on("keyup", ".cost_gst_percent", function () {
    ...
    var cost_rate_with_extracharge = (
        Number(costrate) + Number(extra_charge)
    ).toFixed(4);
    var profitrs = (
        (Number(cost_rate_with_extracharge) * Number(profiteval)) /
        Number(100)
    ).toFixed(4);

    var sellingprice = (
        Number(cost_rate_with_extracharge) + Number(profitrs)
    ).toFixed(4);
    $("#selling_price_" + row_id).val(sellingprice);

    var sellgstper = $("#sell_gst_percent_" + row_id).val();
    var sellingstrs = (
        (Number(sellingprice) * Number(sellgstper)) /
        Number(100)
    ).toFixed(4);
    var productmrp = (Number(sellingprice) + Number(sellingstrs)).toFixed(4);
...
$(document).on("keyup", ".profit_percent", function () {
    ...
    var profitval = (
        (Number(cost_rate_with_extracharge) * Number(profitper)) /
        Number(100)
    ).toFixed(4);
    ...
    var selling_mrp_price = (
        Number(cost_rate_with_extracharge) + Number(profitval)
    ).toFixed(4);
    $("#selling_price_" + row_id).val(selling_mrp_price);
    var sellgstper = $("#sell_gst_percent_" + row_id).val();
    var sellingstrs = (
        (Number(selling_mrp_price) * Number(sellgstper)) /
        Number(100)
    ).toFixed(4);
    $("#sell_gst_amount_" + row_id).val(sellingstrs);
    var productmrp = (Number(selling_mrp_price) + Number(sellingstrs)).toFixed(
        4
    );
    $("#product_mrp_" + row_id).val(productmrp);
    $("#offer_price_" + row_id).val(productmrp);
...
$(document).on("keyup", ".extra_charge", function () {
    ...
    var cost_rate = $("#cost_rate_" + row_id).val();
    var extra_charge = $("#extra_charge_" + row_id).val();
    var cost_rate_with_extra = (
        Number(cost_rate) + Number(extra_charge)
    ).toFixed(4);
    var profit_percent = $("#profit_percent_" + row_id).val();
    var profit_amount = (
        (Number(profit_percent) * Number(cost_rate_with_extra)) /
        Number(100)
    ).toFixed(4);
    $("#profit_amount_" + row_id).val(profit_amount);
    var selling_price = (
        Number(cost_rate_with_extra) + Number(profit_amount)
    ).toFixed(4);
    $("#selling_price_" + row_id).val(selling_price);
    var selgstper = $("#sell_gst_percent_" + row_id).val();
    var sellinggstamt = (
        (Number(selling_price) * Number(selgstper)) /
        Number(100)
    ).toFixed(4);
...
$(document).on("keyup", ".sell_gst_percent", function () {
    ...
    var selligdiff = (
        Number(sellingprice) - Number(cost_rate_with_extra)
    ).toFixed(4);
    var sellinggstamt = (
        (Number(sellingprice) * Number(selgstper)) /
        Number(100)
    ).toFixed(4);
    $("#sell_gst_amount_" + row_id).val(sellinggstamt);
...
$(document).on("keyup", ".offer_price", function () {
    ...
    var profitamt = (
        Number(sellingrate) - Number(cost_rate_with_extracharge)
    ).toFixed(4);
    ...
    var profitpernew = (
        Number(ex1) / Number(cost_rate_with_extracharge)
    ).toFixed(4);
